...
- **`dev-orchestrator`** — multi-model subagent-driven workflow (Opus plan + Codex xhigh plan-review loop + Codex high implement + Sonnet verification gate + Opus two-stage code review + review synthesis, auto-commit, loop cap=4 with anti-pingpong / no-progress guards; thread-addressed Codex resume via `--resume-task`).
