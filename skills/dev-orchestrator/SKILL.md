...
**4.1 Opus subagent review (spec then quality).** Requires a `GATE_OK` from Step 3.5 on the current tree state. Dispatch with `./opus-review-prompt.md`. Provides base SHA, plan-file path, current phase id, and the gate summary. **Reviewer escalation**: dispatch with `model: "fable"` instead of `"opus"` when the phase carries SECURITY / DATA_LOSS materiality risks (per the plan's Risks section) or from round 3 of this fused loop onward — same template, same ultrathink rule, Opus fallback per the Model split rule. Subagent reads diff independently and returns a structured verdict:
